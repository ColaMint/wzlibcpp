#pragma once

#include "NumTypes.hpp"
#include <vector>
#include <array>
#include <cmath>
#include "AES/AES.h"

namespace wz {
    inline constexpr u8 aes_key_1[] = {
        0x13, 0x52, 0x2A, 0x5B, 0x08, 0x02, 0x10, 0x60,
        0x06, 0x02, 0x43, 0x0F, 0xB4, 0x4B, 0x35, 0x05,
        0x1B, 0x0A, 0x5F, 0x09, 0x0F, 0x50, 0x0C, 0x1B,
        0x33, 0x55, 0x01, 0x09, 0x52, 0xDE, 0xC7, 0x1E
    };

    inline constexpr u8 aes_key_2[] = {
        0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00,
        0x06, 0x00, 0x00, 0x00, 0xB4, 0x00, 0x00, 0x00,
        0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00,
        0x33, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00
    };

    inline constexpr u8 shuffle_key[] = {
        
    };

    inline constexpr u8 default_key[] = {
        0xC6, 0x50, 0x53, 0xF2, 0xA8, 0x42, 0x9D, 0x7F,
        0x77, 0x09, 0x1D, 0x26, 0x42, 0x53, 0x88, 0x7C
    };

    inline constexpr u8 shuffle_constant[] = {
        0xF2, 0x53, 0x50, 0xC6
    };

    inline constexpr u8 user_key[] = {
        
    };

    inline constexpr u32 offset_key = 0x581C3F6D;

    inline constexpr u32 header_magic = 0x31474B50;

    inline std::vector<u8> get_trimmed_user_key()  {
        std::vector<u8> num(32);
        for (int i = 0; i < 128; i += 16) {
            num[i / 4] = wz::user_key[i];
        }
        return num;
    }

    class MutableKey final {
    public:
        explicit MutableKey() = default;

        explicit MutableKey(const std::array<u8, 4>& new_iv, std::vector<u8> new_aes_key);

        u8& operator[] (size_t index);

    private:
        static constexpr auto batch_size = 0x10000;
        std::array<u8, 4> iv {0, 0, 0, 0};
        std::vector<u8> aes_key;
        std::vector<u8> keys;

        void ensure_key_size(size_t size);
    };


}
